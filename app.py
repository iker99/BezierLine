...
def draw_bezier_curve(debug_image, ctrl_points):
    ...
    coeffs = binomial_coeffs(3)
    for t in np.linspace(0, 1, 100):
        point = compute_bezier_point(t, ctrl_points, coeffs)
        cv.circle(debug_image, point, 2, (0, 0, 255), -1)
...
def get_bezier_ctrl_points(image, brect, handedness, hand_sign_text,
    ...
    if "Close" in hand_sign_text:
        if len(bezier_ctrl_points) < 4:
            ...
            # Check if we're near any control point to move it
            for i, point in enumerate(bezier_ctrl_points):
                dist = euclidian_distance(mid_point, point)
                if dist <= 50:
                    bezier_ctrl_points[i] = mid_point
                    return

    # If we have more than 4 points, remove the oldest one
    if len(bezier_ctrl_points) > 4:
        bezier_ctrl_points.pop(0)
